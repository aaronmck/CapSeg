...
PlotCapsegSegfit <- function(i, res, min=NULL, max=NULL, plot=T) {
...
# min=NULL; max=NULL
	...
	if (plot) {
		if (length(h.d) > 0) {
			...
			plot(1, type="n", main=array.name, xlab="Capseg Probe Intensity", xlim=c(min, max))
		}
...
PlotCapIntensities <- function(i, res, genomic.limits, intensity.lim, use.capseg.mean=T, draw.legend=T) {
	...
	atten.tau1 = Atten(delta.tau[i, 2], res[["em.fit"]][["theta"]][["at.capseg"]])
	atten.tau2 = Atten(delta.tau[i+1, 2], res[["em.fit"]][["theta"]][["at.capseg"]])
	lines(range(pos1), rep(atten.tau1, 2), col=seg1.col[2], lwd=3)
	lines(range(pos2), rep(atten.tau2, 2), col=seg2.col[2], lwd=3)
	
	if (use.capseg.mean) {
		
		cap.seg = as.data.frame(ReadGladMat(capseg.seg.fn, capseg.sample.name, glad.log=TRUE, drop.x=drop.x, drop.y=drop.y, type="capseg", verbose=F)[[1]], stringsAsFactors=F)
		chr = res[["as.res"]][["h.seg.dat"]][["h.capseg.annot"]][[i]][["chr"]]
		cap.seg.chr = cap.seg[cap.seg$Chromosome == chr, ]
		pos.range = range(c(res[["as.res"]][["h.seg.dat"]][["h.capseg.annot"]][[i]][["pos"]], res[["as.res"]][["h.seg.dat"]][["h.capseg.annot"]][[i+1]][["pos"]]))
		start = which(cap.seg.chr$Start.bp <= pos.range[1] & cap.seg.chr$End.bp >= pos.range[1])
		end = which(cap.seg.chr$Start.bp <= pos.range[2] & cap.seg.chr$End.bp >= pos.range[2])
		FindNearestEndpointRow <- function(pos.range.point) {
			
			start.diffs = abs(pos.range.point - cap.seg.chr$Start.bp)
			end.diffs = abs(pos.range.point - cap.seg.chr$End.bp)
			
			start.or.end = which.min(c(min(start.diffs), min(end.diffs)))
			if (start.or.end == 1) {
				return(which.min(abs(pos.range.point - cap.seg.chr$Start.bp)))
			} else {
				return(which.min(abs(pos.range.point - cap.seg.chr$End.bp)))
			}
		}
		
		if (length(start) == 0 	) {
			start = FindNearestEndpointRow(pos.range[1])
		} 
		if (length(end) == 0) {
			end = FindNearestEndpointRow(pos.range[2])
		}
...
# 		coords[,3] = coords[,3] + abs(min(coords[,3]))
...
PlotBpIntensitiesDEPRECATED <- function(i, res, probe.type, seg1.col, seg2.col, use.cap.seg=F) {
	...
	if (probe.type == "cn") {
		...
		seg2.d = PlotCnSegfit(i+1, res, min = intensity.lim[1], max=intensity.lim[2], plot=F)
		ints = c(seg1.d, seg2.d)
		pos1 = res[["as.res"]][["h.seg.dat"]][["h.cn.annot"]][[i]][["pos"]]
		pos2 = res[["as.res"]][["h.seg.dat"]][["h.cn.annot"]][[i+1]][["pos"]]
		pos = c(pos1, pos2)
		seg12 = data.frame(intensity=ints, position=pos, col=c(rep(seg1.col[1], length(seg1.d)),  rep(seg2.col[1], length(seg2.d))), stringsAsFactors=F)
		seg12 = seg12[order(seg12$position), ]
	} else if (probe.type == "snp") {
		seg1.d = PlotSnpSegfit(i, res, min = intensity.lim[1], max=intensity.lim[2], plot=F)
		seg2.d = PlotSnpSegfit(i+1, res, min = intensity.lim[1], max=intensity.lim[2], plot=F)
		ints = cbind(seg1.d, seg2.d)
		pos1 = res[["as.res"]][["h.seg.dat"]][["h.snp.annot"]][[i]][["pos"]]
		pos2 = res[["as.res"]][["h.seg.dat"]][["h.snp.annot"]][[i+1]][["pos"]]
		pos = c(pos1, pos2)
		seg12 = data.frame(t(ints), position=pos, col=c(rep(seg1.col[1], ncol(seg1.d)),  rep(seg2.col[1], ncol(seg2.d))), stringsAsFactors=F)
		seg12 = seg12[order(seg12$position), ]	
	} else {
		MakeAnnot <- function(j, h.d, h.annot, col) {
			
			if (length(h.d[[j]]) > 0) {
				annot = data.frame(position=h.annot[[j]][["pos"]], intensity=as.vector(h.d[[j]]), col=col, stringsAsFactors=F)	
			} else {
				annot = data.frame(position=NULL, intensity=NULL, col=NULL, stringsAsFactors=F)
			} 
			return(annot)
		}
		
		seg1.annot = MakeAnnot(i, res[["as.res"]][['h.seg.dat']][["h.capseg.d"]], res[["as.res"]][['h.seg.dat']][["h.capseg.annot"]], seg1.col[1])
		seg2.annot = MakeAnnot(i+1, res[["as.res"]][['h.seg.dat']][["h.capseg.d"]], res[["as.res"]][['h.seg.dat']][["h.capseg.annot"]], seg2.col[1])
		pos1 = seg1.annot$position
		pos2 = seg2.annot$position
		seg12 = rbind(seg1.annot, seg2.annot)
	}	
	
	
	if (nrow(seg12) == 0) {
		plot(1, type="n", xlab="Genomic Position", ylab="Copy Ratio", xlim=genomic.limits, ylim=intensity.lim, main="")
		return()
	}
	seg12$intensity = pmin(intensity.lim[2], pmax(intensity.lim[1], seg12$intensity))
	if (probe.type == "cn") {
		e.mu1 = e.mu[i,3]
		e.mu2 = e.mu[i+1,3]
# 	e.mu1 = InvAtten(e.mu[i,3], theta[["at"]])
# 	e.mu2 = InvAtten(e.mu[i+1,3], theta[["at"]])
	} else if (probe.type == "capseg") {
...
# 		coords[,3] = coords[,3] + abs(min(coords[,3]))
			...
			main = paste(probe.type, "Chr:", seg.chr, "Segments: ", i, "and", i+1, "\n", array.name)
			plot(seg12$position, seg12$intensity, pch=19, col=seg12$col, cex=.4, main=main, xlab="Genomic Position", ylab="Copy Ratio", ylim = intensity.lim, xlim=genomic.limits)
			for (j in 1:nrow(coords) ) { 
				lines(c(coords[j,1], coords[j,2]), rep(coords[j, 3], 2), lwd=4, col=c(seg1.col[2], seg2.col[2])[(j+1)%%2 + 1], lty=3 )
			}
			if (!is.null(e.mu)) {
				e.mu1 = e.mu[i,3]
				e.mu2 = e.mu[i+1,3]
				lines(range(pos1), rep(e.mu1, 2), lwd=4, col=seg1.col[2] )
				lines(range(pos2), rep(e.mu2, 2), lwd=4, col=seg2.col[2] )
				legend("topright", legend=c("CBS Fit", "Hapseg Fit"), lty=c(3, 1), lwd=4)
			} else {
				legend("topright", legend=c("CBS Fit"), lty=c(3), lwd=4)
			}
			return(NULL)
			
		} else {
# 		e.mu1 = Atten(e.mu[i,3], theta[["at.capseg"]])
# 		e.mu2 = Atten(e.mu[i+1,3], theta[["at.capseg"]])
			e.mu1 = e.mu[i,3]
			e.mu2 = e.mu[i+1,3]
			
		}
	} else {
		e.mu1 = Atten(e.mu[i,3], theta[['at']])
		e.mu2 = Atten(e.mu[i+1,3], theta[["at"]])
	}
	
	if (!is.null(res[["seg.dat"]][["seg.info"]][i,"GC.Content"])) {
		main = paste(probe.type, "Chr:", seg.chr, "Segments: ", i, "and", i+1, 
				"\n Seg1 GC: ", round(res[["seg.dat"]][["seg.info"]][i,"GC.Content"], 2), "Seg2 GC: ", round(res[["seg.dat"]][["seg.info"]][i+1,"GC.Content"], 2))		
	} else {
		main = paste(probe.type, "Chr:", seg.chr, "Segments: ", i, "and", i+1, "\n", array.name)
	}
	...
	if (!is.null(e.mu) ) {
		...
		lines(range(pos1), rep(e.mu1, 2), lwd=4, col=seg1.col[2] )
		lines(range(pos2), rep(e.mu2, 2), lwd=4, col=seg2.col[2] )
# 	probe.den = density(seg12$position, n=5100)
# 	lines(probe.den$x, mean(c(e.mu1, e.mu2)) * probe.den$y / max(probe.den$y), col="grey", lwd=1)
# 	lines(probe.den$x, mean(c(e.mu1, e.mu2)) * probe.den$y / max(probe.den$y), col="black", lwd=2, lty=3)
...
PlotFFit <- function(i, res, conf=.95, plot=F) {
# i = 1; plot=T; res = iams.res; conf=.95
	
	h.seg.dat = res[["as.res"]][["h.seg.dat"]]
	d = h.seg.dat[["gh.wes.allele.d"]][[i]]
	f.hat = res[["em.fit"]][["wes.f"]][i, "f.hat"]
	f.H0.p = res[["em.fit"]][["wes.f"]][i, "p.H0"]
	f.H1.p = res[["em.fit"]][["wes.f"]][i, "p.H1"]
	...
	alt.phase.prob = CapturePhaseProb( alt=d[2,], ref= d[1,], f.hat, out.p=0)
	e.alpha.seg = 1 + sum(d[2,] * exp(alt.phase.prob)) + sum(d[1,] * (1 - exp(alt.phase.prob)))
	e.beta.seg = 1 + sum(d[1,] * exp(alt.phase.prob)) + sum(d[2,] * (1 - exp(alt.phase.prob)))
	if (plot){
		seg.crds = paste(paste(round(range(res[["as.res"]][["h.seg.dat"]][["gh.wes.allele.annot"]][[i]][['pos']]) / 1e6, 2), collapse="-"), "MB")
		curve( dbeta(x, e.alpha.seg, e.beta.seg) , from=0, to=1, add=F, col= post.col, xlab=paste("Minor Allelic Fraction \n",  seg.crds), ylab="Density")
	}
	...
	SegDensityTrace <- function(x) {
		sapply(x, function(x) {
					s = sum(sapply(1:ncol(d), function(i) {
										e.alpha = 1 + d[2,i]
										e.beta = 1 + d[1,i]
										dbeta(x, e.alpha , e.beta) 
									}))
					return(s/ncol(d))
				})
	}
	...
	if (plot) {
		curve(SegDensityTrace(x), from=0, to=1, col=seg.col, add=T)
		legend(cex = .7, "topright", 
			legend=c("F Posterior of seg", paste("F.hat =", signif(f.hat, 2)), 
				"Summed SNP Posteriors", paste(round(conf*100), '% interval')), 
			fill=c(post.col, f.hat.col, seg.col, conf.col))
		gc.string <- ifelse(!is.null(res[["seg.dat"]][["seg.info"]][i, "GC.Content"]), paste("GC Content: ", signif(res[["seg.dat"]][["seg.info"]][i, "GC.Content"], 2), "\n"), "")
		title(paste("Seg:", i, "\n",
			ncol(d), "total snps \n", med.cov, "median coverage \n", 
			gc.string, 
			paste("H0 Prob:", signif(f.H0.p, 2), "\nH1 Prob:", signif(f.H1.p, 2))), cex.main = .75 )
...
PlotAllCapture <- function(res, save = F, subdir="") {
	...
	foreach(i=1:(length(res[["as.res"]][["h.seg.dat"]][[1]]) -1)) %dopar% { loopStatus(i, 1)
		# i <- 1
		# res <- iams.res
		# save <- FALSE
