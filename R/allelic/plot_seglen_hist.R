PlotSeglenHist <- function(D, W, color.by, color.range=NA, x.max=NA,
  ...
  if (length(bin.w) == 1) {
    res <- hist(D, breaks=seq(0,x.max+bin.w, bin.w), plot=F)
    breaks <- res[["breaks"]]
  }  else {
    breaks <- bin.w
  }
  ...
  if (is.na(color.range)) {
    color.range <- range(color.by, na.rm=T)
  }
  ...
  MyBarplot(heights, seg.colors, ylab=ylab, xlab=xlab, x.max, y.max,
             add=FALSE, breaks=breaks, xlim=xlim, border=border,
             sideways=sideways)
  
  ## show data whiskers
  if (data.whiskers) {
    side <- ifelse(!sideways, 1, 2)
    axis(side=side, at=D, labels=F, line= -0.5, col="grey")
    axis(side=side, at=D[!is.na(color.by)], labels=F, line= -0.5, col="red")
      x.axis.line <- 1
    }
  else{
    x.axis.line=NA
